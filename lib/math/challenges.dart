...
final List<ChallengeSet> challengeSets = [
  ChallengeSet('Switcheroo', [
    ...
    Challenge(Formula('<P|Q>'), 2,
        initialLines: [Formula('<~P->Q>')], verified: false),
    Challenge(Formula('<a=b|a=c>'), 2,
        initialLines: [Formula('<~a=b->a=c>')], verified: false),
    Challenge(Formula('<P|~P>'), 2,
        initialLines: [Formula('<~P->~P>')], verified: false),
  ]),
  ChallengeSet('Fantasy, carry over', [
    Challenge(Formula('<P->P>'), 4, verified: false),
    Challenge(Formula('<a=0->a=0>'), 4, verified: false),
    Challenge(Formula('<P->Q>'), 6,
        initialLines: [Formula('Q')], verified: false),
    Challenge(Formula('<(a+b)=0->a=0>'), 6,
        initialLines: [Formula('a=0')], verified: false),
    Challenge(Formula('<P-><Q->Q>>'), 8, verified: false),
    Challenge(Formula('<P-><Q->P>>'), 9, verified: false),
    Challenge(Formula('<~Q->~Q>'), 4, verified: false),
    Challenge(Formula('<P|~P>'), 5, verified: false),
  ]),
...
class Challenge {
  ...
  Challenge(this.goal, this.goalStepCount,
      {this.initialLines = const [], this.verified = true});
